...
var imports = [];
parser.addListener('', function importFound(event) {
    var finalCSSUrl = url.resolve(event.uri, rule.href);

    if (website.cssParsedUrls.indexOf(finalCSSUrl) === -1) {
        imports.push(parseCSSfromUrl(finalCSSUrl, rule.media.mediaText, website));
    }
});

parser.addListener('', function fileEnded(event) {

    if (imports.length > 0) {
       while(imports.length){
           var cssImport = imports.pop();
       }
    }
});
...
function parseCSS(text, cssUrl, media, isInline, website) {
    ...
    try {
        ...
        imports.push({cssUrl: isInline ? "embed" : cssUrl, css: sheet, media: media, cssBody: text, report: report});
        ...
        for (var i = 0; i < sheet.cssRules.length; i++) {
            ...
            if (rule.type === 3 && rule.href) {
                ...
                var finalCSSUrl = url.resolve(cssUrl, rule.href);
