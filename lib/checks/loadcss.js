    ...
    CSSLint.addRule({
        ...
        init: function (parser, reporter) {
            ...
            parser.addListener('import', function importFound(event) {
                //imports are relative to the CSS importing it
                var finalCSSUrl = url.resolve(cssUrl, event.uri.replace('url(', '').replace(')', ''));
                if (website.cssParsedUrls.indexOf(finalCSSUrl) === -1) {
                    imports.push(parseCSSfromUrl(finalCSSUrl, event.media, website));
                }
            });
